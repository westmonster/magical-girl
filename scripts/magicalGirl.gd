...
# MOVEMENT DIRECTIONS MODIFIER #
#Forward Modifier
export(float) var ForwardMultiplier = 1
#Backward Modifier
export(float) var BackwardMultiplier = 1
#Strafe Modifier
export(float) var StrafeMultiplier = 1
...
#########################
# The 3D camera node.
onready var Node_Camera3D = $Camera_Main
# The "Crosshair_Usable" red circle node.
onready var Node_Crosshair_Useable = $Camera2D/Crosshair/Crosshair_Useable
# The Default Muzzle Rotation
onready var Default_Muzzle_Rotation = $Camera_Main/loadout/muzzle.rotation
#	Label	#
# The top-most label, for debugging.
onready var Debug_Label = $Camera2D/DEBUG/Debug_Label
# Label_01 print string.
var Debug_Label_String = "-------------------"
# The Health label.
onready var health_label = $Camera2D/HUD/Panel/timer_icon/health
# The Potion label.
onready var potion_label = $Camera2D/HUD/Panel/potion_icon/Label
# The White keys label.
onready var wht_keys_label = $Camera2D/HUD/Panel/wht_key_icon/Label
# The Red keys label.
onready var red_keys_label = $Camera2D/HUD/Panel/red_key_icon/Label
# The Blue keys label.
onready var blu_keys_label = $Camera2D/HUD/Panel/blu_key_icon/Label
# The Yellow keys label.
onready var ylw_keys_label = $Camera2D/HUD/Panel/ylw_key_icon/Label
# The Score count label
onready var score_label = $Camera2D/HUD/Panel/Score/Label
# The muzzle to shoot stuff
onready var muzzle		= $Camera_Main/loadout/muzzle
# The projectile to shoot from the muzzle
#export(String, "arrow", "blade", "magic", "force") var Projectile	= "force"
var projectile = load("res://scenes/items/projectiles/bubble.tscn")
...
func Fire():
	...
	prj.fire(get_node("Camera_Main/loadout/muzzle/Muzzle").global_transform.basis.z, FinalMoveVel)
...
func ThrowPotion():
	if potions > 0:
		...
		prj.global_transform = get_node("Camera_Main/loadout/muzzle/Muzzle").global_transform
...
func _input(event):
	...
	if(Input.is_action_pressed("Player_PrimaryAction")):
		if(get_node("Camera_Main/loadout").get_child(0).has_method("Action")):
			get_node("Camera_Main/loadout").get_child(0).Action()
	...
	if(Input.is_action_just_pressed("Player_SecondaryAction")):
		if(get_node("Camera_Main/loadout").get_child(0).has_method("SecondaryAction")):
			get_node("Camera_Main/loadout").get_child(0).SecondaryAction()
...
func _physics_process(delta):
	...
	FootPoint = get_node("RayCast").get_collision_point()
	var FootNormal = get_node("RayCast").get_collision_normal()
	var shadow = get_node("Shadow")
	shadow.translation = FootPoint - translation + Vector3(0,0.35,0)
	shadow.rotation = FootNormal.normalized()  - Vector3(PI/2,0,0)
	if(get_node("RayCast").is_colliding()):
		shadow.opacity = (1+(shadow.translation.y  / 10))
	else:
		shadow.opacity = 0
	#print(shadow.opacity)
	...
	if(Pressed_FW and not Pressed_BW):
		TempMoveVel_FWAndBW.x = -DirectionInNormalVec3_FWAndBW.x * FinalWalkVelocity * ForwardMultiplier
		TempMoveVel_FWAndBW.z = -DirectionInNormalVec3_FWAndBW.z * FinalWalkVelocity * ForwardMultiplier
		#get_node("Camera_Main/loadout").get_child(0).playAnimation("walk")
	#	BACKWARDS	#
	elif(not Pressed_FW and Pressed_BW):
		TempMoveVel_FWAndBW.x = DirectionInNormalVec3_FWAndBW.x * FinalWalkVelocity * BackwardMultiplier
		TempMoveVel_FWAndBW.z = DirectionInNormalVec3_FWAndBW.z * FinalWalkVelocity * BackwardMultiplier
		#get_node("Camera_Main/loadout").get_child(0).playAnimation("walk")
	#	BOTH		#
	...
	if(Pressed_LEFT and not Pressed_RIGHT):
		TempMoveVel_LeftAndRight.x = -DirectionInNormalVec3_LeftAndRight.x * FinalWalkVelocity * StrafeMultiplier
		TempMoveVel_LeftAndRight.z = DirectionInNormalVec3_LeftAndRight.z * FinalWalkVelocity * StrafeMultiplier
		#get_node("Camera_Main/loadout").get_child(0).playAnimation("walk")
	#	RIGHT		#
	...
	if(!Pressed_BW and !Pressed_FW and !Pressed_RIGHT and !Pressed_LEFT):
		get_node("Camera_Main/loadout").get_child(0).playAnimation("idle")
	
	if((Pressed_BW or Pressed_FW or Pressed_RIGHT or Pressed_LEFT) and !Input.is_action_pressed(String_Shift)):
		get_node("Camera_Main/loadout").get_child(0).playAnimation("walk")
	
	if((Pressed_BW or Pressed_FW or Pressed_RIGHT or Pressed_LEFT) and Input.is_action_pressed(String_Shift)):
		get_node("Camera_Main/loadout").get_child(0).playAnimation("run")
	...
	if(Mouse_Moved):
		...
		# Move Loadout Node forward and up if aiming up
		#print(Node_Camera3D.rotation.x)
		if(Node_Camera3D.rotation.x >= 0):
			get_node("Camera_Main/loadout").translation = Vector3(0,1.5,8) * (Node_Camera3D.rotation.x/90)
		if(Node_Camera3D.rotation.x < 0):
			get_node("Camera_Main/loadout").translation = Vector3(0,2,6) * (Node_Camera3D.rotation.x/90)
	...
	if(Input.is_action_pressed("Player_PrimaryAction")):
		if(get_node("Camera_Main/loadout").get_child(0).has_method("Action")):
			get_node("Camera_Main/loadout").get_child(0).Action()
		Pressed_fire = true
		get_node("Camera_Main/loadout/muzzle/Muzzle").rotate_x(delta/35)
		#print(get_node("Camera_Main/loadout/muzzle/Muzzle").rotation.x)
		if(get_node("Camera_Main/loadout/muzzle/Muzzle").rotation.x > 0.25):
			get_node("Camera_Main/loadout/muzzle/Muzzle").rotation.x = 0.25
		get_node("Camera_Main/loadout/muzzle").rotate_z(delta*5)
		
	if(!Pressed_fire):
		#print(get_node("Camera_Main/loadout/muzzle/Muzzle").rotation.x)
		if(get_node("Camera_Main/loadout/muzzle/Muzzle").rotation.x > 0):
			get_node("Camera_Main/loadout/muzzle/Muzzle").rotate_x(delta/-12)
		if(get_node("Camera_Main/loadout/muzzle/Muzzle").rotation.x < 0):
			get_node("Camera_Main/loadout/muzzle/Muzzle").rotation.x = 0
	...
	if(Input.is_action_just_released("Player_PrimaryAction")):
		Pressed_fire = false
		#get_node("Camera_Main/loadout/muzzle").rotation = Default_Muzzle_Rotation
		#get_node("Camera_Main/loadout/muzzle/Muzzle").rotation = Vector3(0,0,0)
