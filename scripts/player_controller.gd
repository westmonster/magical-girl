...
func _process(_delta: float) -> void:
	move_axis.x = Input.get_action_strength("Player_FW") - Input.get_action_strength("Player_BW")
	move_axis.y = Input.get_action_strength("Player_Right") - Input.get_action_strength("Player_Left")
	if flying:
		fly(_delta)
	else:
		walk(_delta)
...
func walk(delta: float) -> void:
	...
	if is_on_wall():
		...
		velocity.y = moving.y
	#prints("Player speed: ", velocity.length())
...
func camera_rotation() -> void:
	...
	if mouse_axis.length() > 0:
		...
		rot_vec = Vector3(vertical, -horizontal,0)
